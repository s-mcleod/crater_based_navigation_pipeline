...
@torch.jit.script
def unimodular_matrix(matrix1: torch.Tensor) -> torch.Tensor:
    ...
    print(matrix1.shape)
    print(matrix1)
    val = 1.0 / torch.det(matrix1)
    return (torch.sign(val) * torch.pow(torch.abs(val), 1.0 / 3.0))[..., None, None] * matrix1
...
def ellipse_to_conic_matrix(
    ...
    # Ensure no in-place operations occur on tensors that require gradients
