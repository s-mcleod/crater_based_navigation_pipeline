...
def init_projection_matrix(pose, K, PANGU=True):
    # PANGU has a unique way of obtaining the rotation matrix.
    if PANGU:
        ...
        Tm_c = R.from_euler('ZYX',np.array([pose.yaw, pose.pitch, pose.roll]),degrees=True).as_matrix()
        nad = -1*np.eye(3)
        nad[2,2] = 1
        R_diff = np.dot(nad, R.from_euler('ZXZ',np.array([0,pose.pitch,0]),degrees=True).as_matrix().T)
        angle_off_nadir_deg = (180/math.pi) * math.acos(min((np.trace(R_diff)-1),2)/2)
